...
export default function PerformanceChart({ data, totalServices }: PerformanceChartProps) {
  ...
  const COLORS = ['#E94560', '#FFDE00', '#3B82F6', '#2ECC71', '#F59E0B', '#8B5CF6', '#1ABC9C']
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-white rounded-xl border border-[#E9ECEF] shadow-lg p-4">
          <p className="font-semibold text-sm text-[#1A1A2E] mb-2">{label}</p>
  ...
  const PieTooltip = ({ active, payload }: any) => {
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-white rounded-xl border border-[#E9ECEF] shadow-lg p-4">
          <p className="font-semibold text-sm text-[#1A1A2E] mb-2">{payload[0].name}</p>
          <p className="text-sm text-[#E94560]">
    ...
    <div className="bg-white rounded-xl border border-[#E9ECEF] shadow-sm p-4 sm:p-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h3 className="font-semibold text-[#1A1A2E] text-sm sm:text-base">Performance Metrics</h3>
            <p className="text-xs text-gray-400 hidden sm:block">Team performance and productivity</p>
          </div>
        </div>
              ...
              <Tooltip
                content={({ active, payload }) => {
                  if (active && payload && payload.length) {
                    return (
                      <div className="bg-white rounded-xl border border-[#E9ECEF] shadow-lg p-4">
                        <p className="font-semibold text-sm text-[#1A1A2E]">{payload[0]?.payload?.subject}</p>
                        <p className="text-sm text-[#E94560]">
      ...
      {/* Stats */}
      <div className="grid grid-cols-3 gap-3 sm:gap-4 mt-6 pt-4 border-t border-[#E9ECEF]">
        <div className="text-center p-2 sm:p-3 bg-[#FAFAFA] rounded-lg">
          <div className="flex items-center justify-center gap-1 mb-1">
            ...
            <p className="text-[10px] sm:text-xs font-medium text-gray-400 uppercase tracking-wider">Completion</p>
          </div>
        ...
        <div className="text-center p-2 sm:p-3 bg-[#FAFAFA] rounded-lg">
          <div className="flex items-center justify-center gap-1 mb-1">
            ...
            <p className="text-[10px] sm:text-xs font-medium text-gray-400 uppercase tracking-wider">Top Performer</p>
          </div>
          <p className="text-xs sm:text-sm md:text-base font-bold text-[#1A1A2E] truncate max-w-[80px] sm:max-w-none">
        ...
        <div className="text-center p-2 sm:p-3 bg-[#FAFAFA] rounded-lg">
          <div className="flex items-center justify-center gap-1 mb-1">
            ...
            <p className="text-[10px] sm:text-xs font-medium text-gray-400 uppercase tracking-wider">Avg Per Tech</p>
          </div>
          <p className="text-sm sm:text-lg md:text-xl font-bold text-[#3B82F6]">
            {avgPerTech.toFixed(1)}
          </p>
          <p className="text-[10px] sm:text-xs text-gray-400">services each</p>
        </div>
      </div>
    </div>
  )
