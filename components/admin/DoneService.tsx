...
export default function DoneService() {
  ...
  const openDetail = async (svc: any) => {
    ...
    const [timelineRes, docsRes, profileRes, itemsRes, dpRes] =
      await Promise.all([
        supabase
        ...
        supabase
          ...
          .eq("service_order_id", svc.id)
          .order("created_at", { ascending: true }),
        svc.assigned_teknisi_id
          ? supabase
              .from("profiles")
              .select("full_name, role")
              .eq("id", svc.assigned_teknisi_id)
              .single()
        ...
        supabase
          .from("layanan")
          .select("nominal")
          .eq("detail_sku", `DP - Invoice ${svc.invoice_number}`)
          .maybeSingle(),
      ]);
    const dpFromLayanan = dpRes?.data?.nominal || 0;
    const dpFinal =
      dpFromLayanan > 0 ? dpFromLayanan : Number(svc?.down_payment) || 0;
    console.log("📦 DoneService items:", {
      fromDb: itemsRes.data?.length,
      fromSvc: svc.items?.length,
      svcItems: svc.items,
  ...
  const DetailModal = () => {
    ...
    const remaining = svc.final_cost
      ...
      : Math.max(
          0,
          (subtotal || svc.estimated_cost || 0) - dpValue - (svc.discount || 0),
        );

    const beforePhotos = docs.filter(
      (d: any) => d.stage === "initial_condition",
    );
    const duringPhotos = docs.filter(
      (d: any) =>
        d.stage &&
        d.stage !== "initial_condition" &&
        d.stage !== "final_condition",
    );
    const afterPhotos = docs.filter(
      (d: any) => d.stage === "final_condition" || d.stage === "qc",
    );

    return (
      <div
        className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[70] p-3 sm:p-4"
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="flex items-center gap-1.5 mt-0.5">
                  ...
                  <span
                    className={`px-1.5 py-0.5 text-[10px] font-medium rounded-full ${statusBadge[svc.status]?.color || "bg-slate-100 text-slate-700"}`}
                  >
                    {statusBadge[svc.status]?.label || svc.status}
                  </span>
                </div>
              </div>
            </div>
              ...
              <div className="grid sm:grid-cols-2 gap-3">
                ...
                <div className="bg-slate-50 rounded-xl p-4 border border-slate-200">
                  ...
                  <p className="text-sm font-semibold text-slate-900">
                    ...
                    {svc.watch_model ? ` ${svc.watch_model}` : ""}
                  </p>
                  <div className="flex flex-wrap gap-1.5 mt-1">
                    {svc.watch_movement && (
                      <span className="text-[10px] bg-white border border-slate-200 px-2 py-0.5 rounded-md text-slate-600">
                        {svc.watch_movement}
                      </span>
                    )}
                    {svc.watch_condition && (
                ...
                <div className="space-y-2">
                  ...
                  {svc.request && (
                    <div className="bg-white rounded-lg p-3 border border-slate-200">
                      ...
                      <p className="text-sm text-slate-800">{svc.request}</p>
                    </div>
                  )}
                  {svc.notes && (
                    <div className="bg-white rounded-lg p-3 border border-slate-200">
                      ...
                      <p className="text-sm text-slate-800">{svc.notes}</p>
                    </div>
                  )}
                  {svc.completion_notes && (
                    <div className="bg-white rounded-lg p-3 border border-slate-200">
                      <p className="text-[10px] text-slate-400 uppercase tracking-wider mb-0.5">
                        Completion Notes
                      </p>
                  ...
                  {svc.qc_submit_notes && (
                    <div className="bg-white rounded-lg p-3 border border-l-4 border-l-amber-400 border-slate-200">
                      <p className="text-[10px] text-amber-600 uppercase tracking-wider mb-0.5 font-semibold">
                        Catatan Teknisi
                      </p>
                      <p className="text-sm text-slate-800">
                        {svc.qc_submit_notes}
                  ...
                  {(svc.warranty_months || svc.warranty_expiry) && (
                    <div className="flex items-center gap-3 text-xs text-slate-500 pt-1">
                      ...
                      {svc.warranty_months && (
                        <span>Garansi {svc.warranty_months} bulan</span>
                      )}
                      {svc.warranty_expiry && (
                        <span>Exp: {fmtDate(svc.warranty_expiry)}</span>
                      )}
                    </div>
                  )}
                </div>
                  ...
                  <div className="space-y-2 max-h-60 overflow-y-auto">
                    {timeline.length === 0 ? (
                      ...
                      timeline.map((ev: any, i: number) => (
                        <div
                          key={ev.id}
                          className="flex items-start gap-3 p-2.5 bg-white rounded-lg border border-slate-200"
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-0.5">
                              ...
                              <span
                                className={`px-1.5 py-0.5 text-[10px] font-medium rounded-full ${statusBadge[ev.status]?.color || "bg-slate-100 text-slate-700"}`}
                              >
  ...
  const contactWA = async (svc: any) => {
    ...
    const totalSparepart = spareparts.reduce(
      (s: number, i: any) => s + (Number(i.price) || 0) * (i.quantity || 1),
      0,
    );
    ...
    lines.push(
      "Jam tangan sudah lolos Quality Control dan sudah dapat diambil.",
    );
    lines.push("");
  ...
  const renderCard = (svc: any, i: number, showDone: boolean) => {
    ...
    const totalCost =
      items.length > 0
        ? items.reduce(
            (s: number, it: any) =>
              s + (parseFloat(it.price) || 0) * (it.quantity || 1),
            0,
          )
          ...
          <div className="flex-1 min-w-0 space-y-2">
            ...
            <div className="flex items-center gap-3 flex-wrap text-sm">
              ...
              <span className="flex items-center gap-1.5">
                ...
                <span className="text-slate-600">{svc.customer_phone}</span>
              </span>
              {svc.watch_brand && (
                <span className="flex items-center gap-1.5">
                  <Watch className="w-4 h-4 text-slate-400" />
                  <span className="text-slate-600">{svc.watch_brand}</span>
                </span>
              )}
            </div>
            {/* Rincian item final */}
            {items.length > 0 && (
              <div className="flex flex-wrap gap-1">
                {items.map((it: any, idx: number) => (
                  <span
                    key={idx}
                    className="inline-flex items-center gap-1 px-2 py-0.5 bg-slate-100 rounded-md text-[11px] text-slate-700 border border-slate-200"
                  >
            ...
            <div className="flex items-center gap-3 text-xs text-slate-500 flex-wrap">
              {svc.issue_description && (
                <span className="line-clamp-1">{svc.issue_description}</span>
              )}
              {items.length > 0 && (
              ...
              {totalCost > 0 && (
                <span
                  className={`font-semibold ${remaining === 0 ? "text-green-600" : "text-blue-600"}`}
                >
                  {remaining === 0
                    ? "✓ LUNAS"
                    : `Sisa: ${fmtRupiah(remaining)}`}
                </span>
              )}
            </div>
          </div>
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">
              Service selesai QC, siap pengambilan
            </p>
          </div>
        </div>
        <div className="relative w-full sm:w-64">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-slate-900/10 focus:border-slate-900"
          />
        </div>
      </motion.div>
        ...
        {loading ? (
          ...
          <div className="text-center py-12 text-slate-400">
            ...
            <p className="text-sm font-medium">
              {tab === "pending"
                ? "Tidak ada service siap diambil"
                : "Belum ada riwayat"}
            </p>
          </div>
        ) : (
