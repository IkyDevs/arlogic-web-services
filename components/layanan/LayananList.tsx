    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm hover:shadow-md transition-all">
          ...
          <p className="text-2xl font-bold text-slate-900">{filteredLayanan.length}</p>
          <p className="text-xs text-slate-400 mt-1">Transaksi</p>
        </div>

        {/* Card 2: Total Amount (Full Width di Mobile) */}
        <div className="bg-gradient-to-br from-blue-600/5 to-blue-600/10 rounded-xl border border-blue-600/20 p-4 shadow-sm hover:shadow-md transition-all col-span-2 md:col-span-1">
          <div className="flex items-center justify-between mb-1">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm hover:shadow-md transition-all">
          ...
          <p className="text-2xl font-bold text-slate-900">{layanan.filter(l => l.status === 'active').length}</p>
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm hover:shadow-md transition-all">
          ...
          <p className="text-2xl font-bold text-slate-900">{layanan.filter(l => l.status === 'completed').length}</p>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm">
        ...
        {showFilters && (
          <motion.div
            ...
            className="mt-4 pt-4 border-t border-slate-200 grid grid-cols-2 md:grid-cols-5 gap-3"
          >
            <div>
            ...
            <div>
              <label className="block text-xs font-medium text-slate-400 uppercase tracking-wider mb-1.5">Payment Method</label>
              <select
                value={filterMetode}
            ...
            <div>
              <label className="block text-xs font-medium text-slate-400 uppercase tracking-wider mb-1.5">From Date</label>
              <input
                type="date"
                value={startDate}
            ...
            <div>
              <label className="block text-xs font-medium text-slate-400 uppercase tracking-wider mb-1.5">To Date</label>
              <input
                type="date"
                value={endDate}
          ...
          <table className="w-full min-w-[1100px]">
            <thead className="bg-slate-50">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">Handled By</th>
                ...
                <th className="px-4 py-3 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">Photo</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">Status</th>
                {isAdmin && <th className="px-4 py-3 text-left text-xs font-medium text-slate-400 uppercase tracking-wider">Actions</th>}
              </tr>
            </thead>
              ...
              {filteredLayanan.map((item, index) => (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    ...
                    <p className="text-xs text-slate-400">{item.customer_whatsapp}</p>
                    {item.detail_sku && (
                      <p className="text-[10px] text-slate-400 mt-0.5">SKU: {item.detail_sku}</p>
                    )}
                  </td>
                  <td className="px-4 py-3">
      ...
      {filteredLayanan.length > 0 && (
        <div className="bg-white rounded-xl border border-slate-200 p-4 shadow-sm flex flex-col sm:flex-row justify-between items-center gap-3">
          <div className="text-center sm:text-left">
            <p className="text-xs text-slate-400 uppercase tracking-wider">Showing</p>
            <p className="font-medium text-slate-900">{filteredLayanan.length} of {layanan.length} transactions</p>
          </div>
          <div className="text-center sm:text-right w-full sm:w-auto">
            <p className="text-xs text-slate-400 uppercase tracking-wider">Total Amount</p>
            <p className="text-xl sm:text-2xl font-bold text-blue-600">{formatRupiah(totalNominal)}</p>
          </div>
        </div>
      )}
          ...
          <div className="max-w-2xl w-full" onClick={(e) => e.stopPropagation()}>
            <div className="bg-white rounded-xl overflow-hidden border border-slate-200 shadow-xl">
              ...
              <div className="p-4 border-t border-slate-200 flex justify-between items-center">
                <p className="text-xs text-slate-400">Transaction Proof</p>
                <button
                  onClick={() => setSelectedPhoto(null)}
                  className="px-4 py-1.5 bg-slate-900 text-white rounded-lg hover:bg-slate-700 transition-all text-sm font-medium"
                >
                  Close
                </button>
              </div>
