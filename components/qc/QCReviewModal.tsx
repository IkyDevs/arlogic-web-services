...
export default function QCReviewModal({
  ...
  const totalCost = localItems.reduce((sum, item) => sum + (item.price || 0) * (item.quantity || 1), 0)
    ...
    try {
      ...
      if (status === 'approved') {
        ...
        for (const orig of serviceItems) {
          ...
          if (!stillExists) {
            changes.push(`• ${orig.item_type === 'jasa' ? 'Jasa' : 'Sparepart'} "${orig.name}" dihapus`)
          }
        }
        ...
        for (const curr of localItems) {
          ...
          if (orig && orig.price !== curr.price) {
            changes.push(`• Harga ${curr.name}: Rp ${(orig.price || 0).toLocaleString()} → Rp ${(curr.price || 0).toLocaleString()}`)
          }
          if (!orig) {
            changes.push(`• ${curr.item_type === 'jasa' ? 'Jasa' : 'Sparepart'} baru: ${curr.name} Rp ${(curr.price || 0).toLocaleString()}`)
          }
        }
          ...
          <div className="bg-[#F8F9FA] rounded-lg p-4 border border-slate-200">
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <h4 className="text-sm font-semibold">Items ({localItems.length})</h4>
              </div>
            ...
            {expandedSections.items && (
              <div className="space-y-2">
                {localItems.length === 0 ? (
                  <p className="text-sm text-slate-400 text-center py-3">Belum ada item</p>
                ) : localItems.map((item, index) => {
                  const isEditing = editingPrice[index] !== undefined;
                  return (
                    <div key={item.id || index} className="flex items-center justify-between p-2 bg-white rounded border border-slate-200 gap-2">
                        ...
                        {isEditing ? (
                          <>
                            <input type="number"
                              ...
                              onChange={(e) => setEditingPrice({ ...editingPrice, [index]: parseInt(e.target.value) || 0 })}
                              ...
                              onKeyDown={(e) => e.key === 'Enter' && savePrice(index)} />
                            <button onClick={() => savePrice(index)} className="p-0.5 text-green-600 hover:bg-green-50 rounded"><Check className="w-3.5 h-3.5" /></button>
                            <button onClick={() => { const { [index]: _, ...rest } = editingPrice; setEditingPrice(rest); }} className="p-0.5 text-slate-400 hover:bg-slate-100 rounded"><X className="w-3.5 h-3.5" /></button>
                          </>
                        ) : (
                          <>
                            <span className="text-sm font-semibold cursor-pointer hover:text-blue-600 transition-colors"
                              onClick={() => startEditPrice(index, item.price || 0)}>
                              {formatRupiah(item.price * item.quantity)}
                            </span>
                            <button onClick={() => deleteItem(index)} className="p-0.5 text-red-400 hover:text-red-600 hover:bg-red-50 rounded transition-colors">
                ...
                {showAddJasa ? (
                  <div className="p-3 bg-white rounded border border-blue-200 space-y-2">
                    <p className="text-xs font-semibold text-blue-700">Tambah Jasa Custom</p>
                    <div className="flex gap-2">
                      <input type="text" value={customJasa.name} onChange={(e) => setCustomJasa({ ...customJasa, name: e.target.value })}
                        placeholder="Nama jasa..." className="flex-1 px-2 py-1.5 text-xs border border-slate-200 rounded focus:outline-none focus:ring-2 focus:ring-blue-500/20" />
                      <input type="number" value={customJasa.price || ''} onChange={(e) => setCustomJasa({ ...customJasa, price: parseInt(e.target.value) || 0 })}
                        placeholder="Harga" className="w-24 px-2 py-1.5 text-xs border border-slate-200 rounded focus:outline-none focus:ring-2 focus:ring-blue-500/20 [appearance:textfield]" />
                      <button onClick={addCustomJasa} disabled={!customJasa.name.trim() || customJasa.price <= 0}
                        className="px-3 py-1.5 bg-blue-600 text-white text-xs font-medium rounded hover:bg-blue-700 disabled:opacity-50 transition-all">Tambah</button>
                      <button onClick={() => setShowAddJasa(false)} className="px-2 py-1.5 text-xs text-slate-500 hover:bg-slate-100 rounded">Batal</button>
                    </div>
                  </div>
                ) : (
