...
import {
  ...
  CheckCircle, XCircle, Check, Trash2, Plus, ZoomIn, Download, ClipboardCheck, Hash, Tag, Award,
} from "lucide-react";
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h3 className="text-base font-bold text-[var(--color-text)]">Review Service</h3>
                {hasDraftData && <span className="text-[10px] bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full font-medium">Draft</span>}
              </div>
              <div className="flex items-center gap-1.5 mt-0.5">
                <span className="text-xs font-mono text-[var(--color-text-tertiary)]">{service.invoice_number}</span>
                <span className="text-[10px] text-[var(--color-text-tertiary)]">•</span>
                <span className="text-xs text-[var(--color-text-secondary)]">{service.teknisi_name}</span>
                <span className="text-[10px] bg-yellow-100 text-yellow-700 px-2 py-0.5 rounded-full font-medium">QC Pending</span>
              </div>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button onClick={onClose} className="p-1.5 hover:bg-[var(--color-surface)] rounded-lg transition-all text-[var(--color-text-tertiary)] hover:text-[var(--color-text)]">
              <X className="w-5 h-5" />
            </button>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div className="bg-[var(--color-surface)] rounded-xl p-4 border border-[var(--color-border)]">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h4 className="text-xs font-semibold text-[var(--color-text-secondary)] uppercase tracking-wider">Customer</h4>
              </div>
              <div className="space-y-2">
                <p className="text-sm font-semibold text-[var(--color-text)]">{service.customer_name}</p>
                <div className="space-y-1">
                  {[{ label: "WhatsApp", value: service.customer_phone || "-", icon: Phone },
                    { label: "Serial", value: service.serial_number || "-", icon: Hash },
                    { label: "Tanggal", value: formatDate(service.created_at), icon: Calendar },
                  ].map((d) => (
                    <div key={d.label} className="flex items-center gap-2 text-xs text-[var(--color-text-tertiary)]">
            ...
            <div className="bg-[var(--color-surface)] rounded-xl p-4 border border-[var(--color-border)]">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h4 className="text-xs font-semibold text-[var(--color-text-secondary)] uppercase tracking-wider">Jam Tangan</h4>
              </div>
              <div className="space-y-2">
                <p className="text-sm font-semibold text-[var(--color-text)]">
                  {service.watch_brand || service.device_brand || "-"}
                  {service.watch_model && <span className="font-normal text-[var(--color-text-secondary)]"> {service.watch_model}</span>}
                </p>
          ...
          <div className="bg-[var(--color-surface)] rounded-xl p-4 border border-[var(--color-border)]">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h4 className="text-xs font-semibold text-[var(--color-text-secondary)] uppercase tracking-wider">Teknisi</h4>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {[{ label: "Teknisi", value: service.teknisi_name || "-" },
                { label: "Start", value: service.start_date ? formatDate(service.start_date) : "-" },
                { label: "Done", value: service.done_date ? formatDate(service.done_date) : "-" },
                { label: "Duration", value: service.work_duration || "-" },
              ].map((d) => (
                <div key={d.label} className="bg-[var(--color-card)] rounded-lg p-2.5 border border-[var(--color-border)]">
                  <p className="text-[10px] text-[var(--color-text-tertiary)] uppercase tracking-wider">{d.label}</p>
                  <p className="text-sm font-semibold text-[var(--color-text)] mt-0.5">{d.value}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-[var(--color-surface)] rounded-xl p-4 border border-[var(--color-border)]">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h4 className="text-xs font-semibold text-[var(--color-text-secondary)] uppercase tracking-wider">Pembayaran</h4>
            </div>
            <div className="grid md:grid-cols-2 gap-3">
              <div className="bg-[var(--color-card)] rounded-xl p-4 border border-[var(--color-border)] space-y-2">
                <div className="flex justify-between items-center">
                  <span className="text-xs text-[var(--color-text-tertiary)]">Estimasi Biaya</span>
                  <span className="text-sm font-semibold text-[var(--color-text)]">{service.estimated_cost ? formatRupiah(service.estimated_cost) : "-"}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-xs text-[var(--color-text-tertiary)]">Total Item</span>
                  <span className="text-sm font-semibold text-[var(--color-text)]">{formatRupiah(totalBeforeDiscount)}</span>
                </div>
                {discount > 0 && (
                  <div className="flex justify-between items-center">
                    <span className="text-xs text-[var(--color-text-tertiary)]">Diskon</span>
                    <span className="text-sm font-semibold text-red-500">-{formatRupiah(effectiveDiscount)}</span>
                  </div>
                )}
              </div>
              <div className="bg-gradient-to-br from-emerald-600 to-emerald-500 rounded-xl p-4 text-white space-y-2 shadow-sm">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-sm font-bold">{service?.down_payment > 0 ? formatRupiah(Number(service.down_payment)) : "Belum DP"}</span>
                </div>
                <div className="h-px bg-white/20" />
                <div className="flex justify-between items-center">
                  <span className="text-xs text-white/80">Sisa yang harus dibayar</span>
          ...
          <div className="bg-[var(--color-surface)] rounded-xl p-4 border border-[var(--color-border)]">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h4 className="text-xs font-semibold text-[var(--color-text-secondary)] uppercase tracking-wider">Service Details</h4>
            </div>
            <div className="space-y-3">
              <div className="bg-[var(--color-card)] rounded-lg p-3 border border-[var(--color-border)]">
                <p className="text-[10px] text-[var(--color-text-tertiary)] uppercase tracking-wider mb-1">Issue</p>
                <p className="text-sm text-[var(--color-text)] leading-relaxed">{service.issue_description}</p>
              </div>
              {service.request && (
                <div className="bg-[var(--color-card)] rounded-lg p-3 border border-[var(--color-border)]">
                  <p className="text-[10px] text-[var(--color-text-tertiary)] uppercase tracking-wider mb-1">Customer Request</p>
                  <p className="text-sm text-[var(--color-text)] leading-relaxed">{service.request}</p>
                </div>
              )}
              {service.notes && (
                <div className="bg-[var(--color-card)] rounded-lg p-3 border border-[var(--color-border)]">
                  <p className="text-[10px] text-[var(--color-text-tertiary)] uppercase tracking-wider mb-1">Catatan</p>
                  <p className="text-sm text-[var(--color-text)] leading-relaxed">{service.notes}</p>
                </div>
              )}
              {service.completion_notes && (
                <div className="bg-[var(--color-card)] rounded-lg p-3 border border-[var(--color-border)]">
                  <p className="text-[10px] text-[var(--color-text-tertiary)] uppercase tracking-wider mb-1">Completion Notes</p>
                  <p className="text-sm text-[var(--color-text)] leading-relaxed">{service.completion_notes}</p>
                </div>
              )}
              {service.qc_submit_notes && (
                <div className="bg-[var(--color-card)] rounded-lg p-3 border border-[var(--color-border)] border-l-4 border-l-amber-400">
                  <p className="text-[10px] text-amber-600 uppercase tracking-wider mb-1 font-semibold">Catatan Teknisi</p>
                  <p className="text-sm text-[var(--color-text)] leading-relaxed">{service.qc_submit_notes}</p>
                </div>
              )}
              {(service.warranty_months || service.warranty_expiry) && (
                <div className="flex items-center gap-4 pt-2 border-t border-[var(--color-border)]">
                  <div className="flex items-center gap-1.5 text-xs text-[var(--color-text-tertiary)]">
                    ...
                    {service.warranty_months ? `Garansi ${service.warranty_months} bulan` : ""}
                  </div>
                  {service.warranty_expiry && (
                    <span className="text-xs text-[var(--color-text-tertiary)]">Exp: {formatDate(service.warranty_expiry)}</span>
                  )}
                </div>
              )}
            </div>
          ...
          {serviceItems.length > 0 && (
            <div className="bg-[var(--color-surface)] rounded-xl p-4 border border-[var(--color-border)] border-l-4 border-l-amber-400">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h4 className="text-xs font-semibold text-[var(--color-text-secondary)] uppercase tracking-wider">Item dari Teknisi</h4>
                <span className="text-[10px] bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full font-medium">{serviceItems.length} item</span>
              </div>
                  ...
                  return (
                    <div key={item.id || index}
                      ...
                      <div className="flex items-center gap-2 min-w-0 flex-1">
                        <span className={`px-1.5 py-0.5 text-[10px] font-semibold rounded-md flex-shrink-0 ${item.item_type === "jasa" ? "bg-pink-100 text-pink-700" : "bg-purple-100 text-purple-700"}`}>
                      ...
                      <div className="flex items-center gap-2 flex-shrink-0">
                        ...
                        {isDeleted && <span className="text-[10px] font-medium text-red-600 bg-red-100 px-1.5 py-0.5 rounded-md">Dihapus</span>}
                        {priceChanged && <span className="text-[10px] font-medium text-amber-600 bg-amber-100 px-1.5 py-0.5 rounded-md">Harga berubah</span>}
                        {qtyChanged && !priceChanged && <span className="text-[10px] font-medium text-amber-600 bg-amber-100 px-1.5 py-0.5 rounded-md">Qty berubah</span>}
                      </div>
                    </div>
                  );
                })}
              ...
              {localItems.some((li) => li.id?.toString().startsWith("custom_")) && (
                <p className="text-xs text-amber-600 mt-2 flex items-center gap-1 bg-amber-50 rounded-lg p-2">
                  <span className="font-bold">*</span> Item baru yang ditambahkan QC tidak muncul di daftar ini
                </p>
              )}
            </div>
          )}
          ...
          {(service.po_status || service.po_sparepart) && (
            <div className="bg-[var(--color-surface)] rounded-xl p-4 border border-[var(--color-border)]">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h4 className="text-xs font-semibold text-[var(--color-text-secondary)] uppercase tracking-wider">Purchase Order</h4>
              </div>
              <div className="grid grid-cols-2 gap-2">
                {[
                  { label: "Status PO", value: service.po_status },
                  { label: "Sparepart PO", value: service.po_sparepart },
                  { label: "Requested At", value: service.po_requested_at ? formatDate(service.po_requested_at) : null },
          ...
          <div className="bg-[var(--color-surface)] rounded-xl p-4 border border-[var(--color-border)]">
            <button onClick={() => toggleSection("items")} className="w-full flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <h4 className="text-xs font-semibold text-[var(--color-text-secondary)] uppercase tracking-wider">Items</h4>
                <span className="text-[10px] bg-[var(--color-card)] text-[var(--color-text-tertiary)] px-2 py-0.5 rounded-full border border-[var(--color-border)]">{localItems.length} item</span>
              </div>
              {expandedSections.items ? <ChevronDown className="w-4 h-4 text-[var(--color-text-tertiary)]" /> : <ChevronRight className="w-4 h-4 text-[var(--color-text-tertiary)]" />}
            </button>
              ...
              <div className="space-y-2">
                {localItems.length === 0 ? (
                  ...
                  localItems.map((item, index) => {
                    ...
                    return (
                      <div key={item.id || index} className={`flex items-center justify-between p-2.5 rounded-lg border gap-2 transition-all ${
                        isEditing ? "bg-blue-50 border-blue-200 shadow-xs" : "bg-[var(--color-card)] border-[var(--color-border)]"
                      }`}>
                        <div className="flex items-center gap-2 min-w-0 flex-1">
                          <span className={`px-1.5 py-0.5 text-[10px] font-semibold rounded-md flex-shrink-0 ${item.item_type === "jasa" ? "bg-pink-100 text-pink-700" : "bg-purple-100 text-purple-700"}`}>
                            {item.item_type === "jasa" ? "JASA" : "SPR"}
                        ...
                        <div className="flex items-center gap-1 flex-shrink-0">
                          {isEditing ? (
                            <>
                              <button onClick={() => saveItemEdit(index)} className="p-1 text-green-600 hover:bg-green-100 rounded-lg transition-colors" title="Simpan"><Check className="w-4 h-4" /></button>
                              <button onClick={() => cancelItemEdit(index)} className="p-1 text-[var(--color-text-tertiary)] hover:bg-[var(--color-surface)] rounded-lg transition-colors" title="Batal"><X className="w-4 h-4" /></button>
                            </>
                          ) : (
                            <>
                              <span className="text-xs font-semibold text-[var(--color-text)] cursor-pointer hover:text-[var(--color-accent)] bg-[var(--color-surface)] px-2 py-1 rounded-md" onClick={() => startEditItem(index, item)}>
                                {formatRupiah((item.price || 0) * (item.quantity || 1))}
                              </span>
                              <button onClick={() => deleteItem(index)} className="p-1 text-red-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Hapus"><Trash2 className="w-3.5 h-3.5" /></button>
                            </>
                          )}
                        </div>
                      </div>
                ...
                <div className="space-y-1.5 p-4 bg-gradient-to-br from-[var(--color-primary)] to-[var(--color-secondary)] text-white rounded-xl shadow-sm">
                  ...
                  <div className="flex justify-between items-center gap-2">
                    <span className="text-sm font-medium text-white/90">Diskon</span>
                    <div className="flex items-center gap-1">
                      ...
                      <span className="text-xs text-white/60">({discountPercent}%)</span>
                    </div>
                  </div>
                  ...
                  <div className="p-3.5 bg-blue-50 rounded-xl border border-blue-200 space-y-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                      <input type="text" value={customJasa.name} onChange={(e) => setCustomJasa({ ...customJasa, name: e.target.value })}
                        placeholder="Nama jasa..." className="min-w-[120px] flex-1 px-3 py-2 text-xs border border-blue-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 bg-white" />
                      <input type="number" value={customJasa.quantity || ""} onChange={(e) => setCustomJasa({ ...customJasa, quantity: parseInt(e.target.value) || 1 })}
                        placeholder="Qty" className="w-16 px-2 py-2 text-xs border border-blue-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 bg-white text-center" />
                      <input type="number" value={customJasa.price || ""} onChange={(e) => setCustomJasa({ ...customJasa, price: parseInt(e.target.value) || 0 })}
                        placeholder="Harga" className="w-24 px-2 py-2 text-xs border border-blue-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 bg-white text-right" />
                      <button onClick={addCustomJasa} disabled={!customJasa.name.trim() || customJasa.price <= 0}
                        className="px-4 py-2 bg-blue-600 text-white text-xs font-semibold rounded-lg hover:bg-blue-700 disabled:opacity-50 transition-colors shadow-xs">Tambah</button>
                      <button onClick={() => setShowAddJasa(false)} className="px-3 py-2 text-xs font-medium text-[var(--color-text-secondary)] hover:bg-blue-100 rounded-lg transition-colors">Batal</button>
                    </div>
                  </div>
                ) : (
                ...
                {showAddSparepart ? (
                  <div className="p-3.5 bg-purple-50 rounded-xl border border-purple-200 space-y-3">
                    <p className="text-xs font-semibold text-purple-700 flex items-center gap-1.5">
                      <Plus className="w-3 h-3" /> Tambah Sparepart
                    </p>
                    <div className="flex flex-wrap gap-2">
                      <input type="text" value={customSparepart.name} onChange={(e) => setCustomSparepart({ ...customSparepart, name: e.target.value })}
                        placeholder="Nama sparepart..." className="min-w-[120px] flex-1 px-3 py-2 text-xs border border-purple-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500/20 bg-white" />
                      <input type="number" value={customSparepart.quantity || ""} onChange={(e) => setCustomSparepart({ ...customSparepart, quantity: parseInt(e.target.value) || 1 })}
                        placeholder="Qty" className="w-16 px-2 py-2 text-xs border border-purple-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500/20 bg-white text-center" />
                      <input type="number" value={customSparepart.price || ""} onChange={(e) => setCustomSparepart({ ...customSparepart, price: parseInt(e.target.value) || 0 })}
                        placeholder="Harga" className="w-24 px-2 py-2 text-xs border border-purple-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500/20 bg-white text-right" />
                      <button onClick={addCustomSparepart} disabled={!customSparepart.name.trim() || customSparepart.price <= 0}
                        className="px-4 py-2 bg-purple-600 text-white text-xs font-semibold rounded-lg hover:bg-purple-700 disabled:opacity-50 transition-colors shadow-xs">Tambah</button>
                      <button onClick={() => setShowAddSparepart(false)} className="px-3 py-2 text-xs font-medium text-[var(--color-text-secondary)] hover:bg-purple-100 rounded-lg transition-colors">Batal</button>
                    </div>
                  </div>
                ) : (
          ...
          {/* Timeline */}
          <div className="bg-[var(--color-surface)] rounded-xl p-4 border border-[var(--color-border)]">
            <button onClick={() => toggleSection("timeline")} className="w-full flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <h4 className="text-xs font-semibold text-[var(--color-text-secondary)] uppercase tracking-wider">Aktivitas Teknisi</h4>
                <span className="text-[10px] bg-[var(--color-card)] text-[var(--color-text-tertiary)] px-2 py-0.5 rounded-full border border-[var(--color-border)]">{timeline.length} event</span>
              </div>
              {expandedSections.timeline ? <ChevronDown className="w-4 h-4 text-[var(--color-text-tertiary)]" /> : <ChevronRight className="w-4 h-4 text-[var(--color-text-tertiary)]" />}
            </button>
            {expandedSections.timeline && (
              <div className="space-y-2 pt-1">
                {timeline.length === 0 ? (
                  <p className="text-sm text-[var(--color-text-tertiary)] text-center py-4 bg-[var(--color-card)] rounded-lg border border-[var(--color-border)]">Tidak ada aktivitas tercatat.</p>
                ) : (
                  timeline.map((event, index) => (
                    <div key={event.id || index} className="flex items-start gap-3 p-3 bg-[var(--color-card)] rounded-lg border border-[var(--color-border)]">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-0.5">
                          <span className="text-[10px] text-[var(--color-text-tertiary)] font-mono">{formatDate(event.created_at)}</span>
                          <span className={`px-1.5 py-0.5 text-[10px] font-medium rounded-full ${getStatusBadge(event.status).color}`}>{getStatusBadge(event.status).label}</span>
                        </div>
                        <p className="text-sm text-[var(--color-text)] font-medium">{event.message}</p>
                        {event.details?.spareparts && event.details.spareparts.length > 0 && (
                          <div className="mt-2 p-2 bg-purple-50 rounded-lg border border-purple-200">
                            <p className="text-[10px] font-semibold text-purple-700 uppercase tracking-wider mb-1">Item Sparepart yang Ditambahkan:</p>
                            <ul className="space-y-0.5">
                              {event.details.spareparts.map((sp: any, spIndex: number) => (
                                <li key={spIndex} className="text-xs text-purple-700 flex items-center gap-1">
                                  <span className="w-1 h-1 bg-purple-400 rounded-full" />
                                  {sp.name} ({sp.qty}x) @ Rp{sp.price.toLocaleString("id-ID")}
                                </li>
                              ))}
                            </ul>
                          </div>
                        )}
                        {event.details?.action === "qc_price_changes" && event.details.changes && event.details.changes.length > 0 && (
                          <div className="mt-2 p-2 bg-amber-50 rounded-lg border border-amber-200">
                            <p className="text-[10px] font-semibold text-amber-700 uppercase tracking-wider mb-1">Perubahan Item oleh QC:</p>
                            <ul className="space-y-0.5">
                              {event.details.changes.map((change: string, changeIndex: number) => (
                                <li key={changeIndex} className="text-xs text-amber-700 flex items-center gap-1">
            ...
            <div className="bg-[var(--color-surface)] rounded-xl p-4 border border-[var(--color-border)]">
              <button onClick={() => toggleSection("photos")} className="w-full flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <h4 className="text-xs font-semibold text-[var(--color-text-secondary)] uppercase tracking-wider">Dokumentasi</h4>
                  <span className="text-[10px] bg-[var(--color-card)] text-[var(--color-text-tertiary)] px-2 py-0.5 rounded-full border border-[var(--color-border)]">{documentations.length} foto</span>
                </div>
                {expandedSections.photos ? <ChevronDown className="w-4 h-4 text-[var(--color-text-tertiary)]" /> : <ChevronRight className="w-4 h-4 text-[var(--color-text-tertiary)]" />}
              </button>
              {expandedSections.photos && (
                <div className="grid grid-cols-3 md:grid-cols-4 gap-2">
                  {documentations.map((doc, index) => (
                    <div key={doc.id} className="relative group cursor-pointer rounded-xl overflow-hidden border border-[var(--color-border)] aspect-square bg-[var(--color-card)]">
                      ...
                      <div className="absolute inset-0 bg-gradient-to-t from-black/30 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-1.5">
                        <span className="text-[10px] text-white font-medium bg-black/50 px-1.5 py-0.5 rounded-md">{doc.stage || "Progress"}</span>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          <div className="bg-[var(--color-surface)] rounded-xl p-4 border border-[var(--color-border)]">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h4 className="text-xs font-semibold text-[var(--color-text-secondary)] uppercase tracking-wider">Review Notes</h4>
              <span className="text-[10px] text-red-500 font-medium">*wajib untuk penolakan</span>
            </div>
