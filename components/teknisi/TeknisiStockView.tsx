...
export default function TeknisiStockView() {
  ...
  const fetchStock = useCallback(async () => {
    ...
    try {
      const { data } = await supabase
        .from("inventory")
        .select("id, item_name, sku, store_stock, warehouse_stock, min_stock, price, buy_price, unit, category")
        .order("item_name");
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h3 className="text-lg font-bold text-gray-900 dark:text-gray-100">Stock Toko {user?.branch_id ? "" : ""}</h3>
          <p className="text-xs text-gray-500">Stok sparepart di toko cabangmu (read-only)</p>
        </div>
        <div className="relative">
          ...
          <input
            ...
            className="pl-9 pr-4 py-2 bg-white dark:bg-[#1c1c1c] border border-gray-200 dark:border-white/10 rounded-xl text-sm w-56 sm:w-72"
          />
        </div>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-200 dark:border-white/10 text-left text-[10px] text-gray-500 uppercase tracking-wider">
                ...
                <th className="px-4 py-2.5">Stok Toko</th>
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-white/5">
              {filtered.map((it) => {
                const low = it.store_stock <= it.min_stock;
                return (
              ...
              {filtered.length === 0 && (
                <tr>
                  <td colSpan={6} className="px-4 py-8 text-center text-gray-400">
                    ...
                    {search ? "Tidak ada hasil" : "Belum ada barang"}
                  </td>
                </tr>
              )}
            </tbody>
