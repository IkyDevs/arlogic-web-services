...
export function useCentralUpload(sessionKey: string): UseCentralUploadReturn {
  ...
  const [uploadSession, setUploadSession] = useState<CreateSessionResponse | null>(null)
  ...
  const addFiles = useCallback(async (
    ...
    if (mountedRef.current) {
      console.log('[DEBUG:useCentralUpload] addFiles calling setPendingFiles', {
      ...
      console.log('[DEBUG:useCentralUpload] addFiles SKIPPED setPendingFiles - mountedRef is FALSE')
    }
    return result
  }, [sessionKey])
  ...
  const uploadToSupabase = useCallback(async (
    ...
    const result = await uploadService.uploadToSupabase(session, files, (completed, total) => {
  ...
  const checkStatus = useCallback(async (sid: string): Promise<UploadSessionStatus> => {
  ...
  const legacyUpload = useCallback(async (
    files: File[],
    type: string,
    caption?: string,
    timeout?: number,
