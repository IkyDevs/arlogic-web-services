...
export async function POST(request: Request) {
  ...
  const { data: service, error: fetchError } = await supabase
    .from("service_orders")
    .select(
      "id, status, assigned_teknisi_id, invoice_number, customer_name, branch_id",
    )
...
export async function GET(request: Request) {
  ...
  const { data: service } = await supabase
    .from("service_orders")
    .select(
      "id, status, assigned_teknisi_id, invoice_number, customer_name, branch_id, watch_brand, device_brand, watch_model, device_model, issue_description, created_at",
    )
    .eq("id", serviceOrderId)
    .single();
  ...
  // Count active projects for this technician (untuk monitoring, bukan validasi)
  ...
  return NextResponse.json({
    ...
    data: {
      service: {
        ...
        watch_brand:
          (service as any).watch_brand || (service as any).device_brand,
        watch_model:
          (service as any).watch_model || (service as any).device_model,
