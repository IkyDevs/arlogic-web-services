...
export async function GET(request: NextRequest) {
  try {
    ...
    // Admin per cabang → scope pencarian ke cabangnya
    const { data: profile } = await supabase
      .from('profiles')
      .select('role, branch_id')
      .eq('id', user.id)
      .single()
    const isBranchScoped = profile?.role === 'admin' && !!profile?.branch_id
    const branchMatch = isBranchScoped ? { branch_id: profile.branch_id } : {}
