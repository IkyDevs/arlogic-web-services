...
const ALLOWED_STATUSES = ["qc_pending", "revision_required"];
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!ALLOWED_STATUSES.includes(service.status)) {
      return errorResponse(
        createApiError(
          ...
          `Cannot retract from QC when service status is '${service.status}'`,
          { currentStatus: service.status, expectedStatus: ALLOWED_STATUSES }
        ),
        409
      );
    }
    ...
    const result = await transitionWithTimeline(
      ...
      user.id,
      "in_progress",
      "Service ditarik kembali untuk revisi",
      undefined,
      { action: "retract_qc" }
    );
    ...
    return successResponse(
      ...
      "Service berhasil ditarik kembali"
    );
  } catch (error) {
    ...
    console.error("Retract QC error:", error);
    return errorResponse(
