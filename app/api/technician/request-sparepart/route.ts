...
import { validateServiceOrderId, validateSparepartRequest } from "@/lib/api/technician/validation";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const sparepartData = validateSparepartRequest(body);
    ...
    if (service.status !== "in_progress") {
      return errorResponse(
        createApiError(
          ...
          `Cannot request sparepart when service status is '${service.status}'`,
    ...
    const result = await transitionWithTimeline(
      ...
      user.id,
      "req_sparepart_admin",
      `Request sparepart: ${sparepartData.sparepart}`,
      {
        po_sparepart: sparepartData.sparepart,
        po_requested_at: new Date().toISOString(),
        teknisi_pending_reason: sparepartData.notes || `Request sparepart: ${sparepartData.sparepart}`,
      },
      {
        action: "request_sparepart",
        sparepart: sparepartData.sparepart,
    ...
    return successResponse(
      ...
      "Request sparepart berhasil dikirim"
    );
  } catch (error) {
    ...
    console.error("Request sparepart error:", error);
    return errorResponse(
